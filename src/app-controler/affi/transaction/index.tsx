...
const df_From = dayjs(new Date()).add(-7, 'day').format('DD-MM-YYYY')
const df_To = dayjs(new Date()).format('DD-MM-YYYY');
    ...
    return (
        <Card className="rounded-2xl">
            ...
            <CardContent className="space-y-6">
                <Card>
                    <TransactionSearch onChangeForm={handleChangeForm} onReset={handleResetForm} searchValue={params.searchValue} onSearch={handleUpdateSearch} listStatus={TYPE_LIST} />
                </Card>
                <TractionTable transactions={listTransaction} />
                <Pagination onChange={(page) => setParams(pre => ({ ...pre, currentPage: page }))} page={params.currentPage} totalPages={totalPages} />
            </CardContent>
        </Card>
    );
}
